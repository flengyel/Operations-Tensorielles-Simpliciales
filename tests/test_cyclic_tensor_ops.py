...
def test_degen_axis_then_cyclic_commutes():
    """
    Verify s_i τ = τ s_{i-1}  for i>0,
           and    s_0 τ = s_d τ^2.
    """
    shape = (3,4,5)    # axis=1 length 4 ⇒ d=3
    T     = random_tensor(shape, seed=7)
    axis  = 1
    d     = shape[axis] - 1
    ...
    for i in range(d+1):
        ...
        assert_array_equal(
            lhs, rhs,
            f"cyclic‐module degen identity failed at i={i}, axis={axis}"
        )
